...
\author{David~Hulme%
\thanks{D. Hulme is a student with the Department
of Electronics and Computer Science, University of Southampton, UK e-mail: dh7g10@soton.ac.uk}}% <-this % stops a space
...
\caption{CPU usage when playing a video across different browsers on low-power laptops. Data produced by averaging results from Ilias et al. \cite{inproceedings:aStudyOfVideoPerformanceAnalysis} across two different CPU monitoring tools and both wired and wireless networks.}
...
\begin{table}
  \caption{WCAG 2.0 Guidelines, conformance levels are graded A (lowest), AA, and AAA (highest) \cite{standard:wcag2}}
...
Accessibility must be considered when including media on a web page. If a link is provided to a video it must meet certain criteria described in WCAG 2.0 Guideline 4.1.2. If it is embedded, there are no relevant WCAG 2.0 guidelines.
